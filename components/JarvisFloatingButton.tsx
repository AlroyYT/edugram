...
const JarvisFloatingButton = () => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.altKey || (e.ctrlKey && e.shiftKey)) {
        ...
        switch (keyPressed) {
          ...
          case 't': // Alt+T for Topic Explorer
            destinationName = "Topic Explorer";
            navigateAction = () => router.push('/topic-explorer');
            useFile = false;
            break;
          ...
          case 'j': // Alt+J for Help with voice commands
            speakForBlindUsers(
              ...
              "Alt+T for Topic Explorer, Alt+S for Summary, Alt+F for Flashcards, Alt+Q for Quiz.",
              true
            );
  ...
  const safelyStartRecognition = (context: string) => {
    ...
    // Don't try to start if we already know it's active based on our component state
    if (isRecognitionActive) {
      console.log(`Recognition already active in ${context} based on component state, skipping start`);
      return true;
    }
    
    // Double-check if recognition might actually be running already
    ...
    try {
      ...
      console.log(`Browser doesn't support checking recognition state directly in ${context}`);
    }
    ...
    if (!isActuallyRunning) {
      try {
        ...
        if (recognitionRef.current._hasInvalidStateError) {
          ...
          // First try to stop the current instance
          try {
            recognitionRef.current.stop();
          } catch (stopErr) {
            console.log("Error stopping recognition before recreation (expected):", stopErr);
          }
        ...
        try {
          ...
          console.log(`Recognition successfully started in ${context}`);
          return true;
        } catch (startErr: any) {
          if (startErr.name === 'InvalidStateError') {
            console.log(`Recognition already started in ${context} (caught via InvalidStateError)`);
            // Avoid immediate recreation as it might cause issues
            // Just mark it for potential recreation on next attempt if issues persist
            recognitionRef.current._hasInvalidStateError = true;
  ...
  // Load conversation from localStorage when component mounts - using a key specific to floating Jarvis
  useEffect(() => {
    const savedConversation = localStorage.getItem('floating_jarvis_conversation');
    if (savedConversation) {
      try {
        setConversation(JSON.parse(savedConversation));
      } catch (e) {
        console.error("Error loading saved floating Jarvis conversation:", e);
        // If there's an error parsing, start with empty conversation
      }
    }
  }, []);

  // Save conversation to localStorage whenever it changes - using a key specific to floating Jarvis
  useEffect(() => {
    if (conversation.length > 0) {
      localStorage.setItem('floating_jarvis_conversation', JSON.stringify(conversation));
    }
  }, [conversation]);
  ...
  useEffect(() => {
    ...
    recognitionRef.current.onerror = (event: any) => {
      ...
      if ((event.error === 'no-speech' || event.error === 'aborted') && isReady) {
        ...
        // Don't show these common errors to the user as they're part of normal operation
        ...
        setTimeout(() => {
          if (recognitionRef.current && !isRecognitionActive && isReady && recognitionInitialized) {
            try {
              ...
              // Then start after a short delay using our safe method
              setTimeout(() => {
                if (recognitionRef.current && !isRecognitionActive && isReady && recognitionInitialized) {
        ...
        // Only show actual errors to the user, not the routine ones
  ...
  const handleAudioEnded = () => {
    ...
    if (remainingChunks.length > 0) {
      ...
      // Check if recognition should be restarted, but don't try to restart it directly here
      // Wait for a moment to let states update
      setTimeout(() => {
        try {
          // @ts-ignore - Some browsers implement this non-standard property
          const actuallyRunning = recognitionRef.current && recognitionRef.current.state === 'running';
          
          if (!actuallyRunning && !isRecognitionActive && isReady && recognitionInitialized) {
            console.log("Recognition not active after audio ended, scheduling restart");
            // Use setTimeout to avoid immediate execution while still in event handler
            setTimeout(() => safelyStartRecognition("after audio ended"), 500);
          } else if (actuallyRunning && !isRecognitionActive) {
            console.log("Recognition actually running but state doesn't match, updating state");
            setIsRecognitionActive(true);
          }
        } catch (err) {
          console.log("Could not check recognition state, using state variable only");
          // Only try to restart if our state indicates it's not active
          if (!isRecognitionActive && isReady && recognitionInitialized) {
            setTimeout(() => safelyStartRecognition("after audio ended"), 500);
          }
        }
      }, 700);
  ...
  const checkForNavigation = (transcript: string) => {
    ...
    if (lowerText.includes('go to flashcards') || lowerText.includes('open flashcards')) {
    ...
    else if (lowerText.includes('go to topic') || 
             lowerText.includes('open topic') || 
             lowerText.includes('topic explorer') || 
             lowerText.includes('personalized learning') ||
             lowerText.includes('personalised learning') ||
             lowerText.includes('discover topics') ||
             lowerText.includes('learning topics')) {
      speakForBlindUsers("Navigating to personalized learning and topic explorer", true);
      router.push('/topic-explorer');
      return true;
    }
    else if (lowerText.includes('go to topic') || lowerText.includes('topic explorer') || lowerText.includes('personalized learning') || lowerText.includes('personalised learning')) {
      speakForBlindUsers("Navigating to personalized learning and topic explorer", true);
      router.push('/topic-explorer');
      return true;
    }
    else if (lowerText.includes('go to features') || lowerText.includes('show features') || lowerText.includes('main features')) {
      speakForBlindUsers("Navigating to features page", true);
      ...
      if (currentPage === '/flash') {
        ...
        speakForBlindUsers("Voice commands: Say 'go to' followed by flashcards, quiz, deaf assistance, home page, upload document, summary, or topic explorer to navigate. For personalized learning, say 'go to topic explorer' or 'personalized learning'. Say 'help' on any page to hear available commands.", true);
      }
      return true;
    }
              ...
              if (audioRef.current) {
                ...
                audioRef.current.play()
                  .catch(err => {
                    ...
                    setTimeout(() => {
                      if (recognitionRef.current && !isRecognitionActive && isReady && recognitionInitialized) {
                        // Use our safe method to restart recognition
                        console.log("Restarting recognition after audio playback error");
                        safelyStartRecognition("after audio error");
                      }
                    }, 1000);
                  });
                ...
                audioRef.current.onended = () => {
                  ...
                  if (data.additional_chunks && data.additional_chunks.length > 0) {
                    ...
                    // Check the actual state of recognition before trying to restart it
                    // Give a moment for state to settle before checking
                    setTimeout(() => {
                      // Verify recognition is not active before trying to start it
                      try {
                        // @ts-ignore - Some browsers implement this non-standard property
                        const actuallyRunning = recognitionRef.current && recognitionRef.current.state === 'running';
                        if (!actuallyRunning && !isRecognitionActive && isReady && recognitionInitialized) {
                          console.log("Restarting recognition after audio playback");
                          safelyStartRecognition("after audio playback");
                        } else if (actuallyRunning) {
                          // Already running, just update our state to match
                          console.log("Recognition already running after audio playback, updating state");
                          setIsRecognitionActive(true);
                        }
                      } catch (err) {
                        // If we can't check state directly, use our helper function which has its own checks
                        if (!isRecognitionActive && isReady && recognitionInitialized) {
                          console.log("Restarting recognition after audio playback");
                          safelyStartRecognition("after audio playback");
                        }
                ...
                audioRef.current.play()
                  .catch(err => {
                    ...
                    setTimeout(() => {
                      if (recognitionRef.current && !isRecognitionActive && isReady && recognitionInitialized) {
                        // Use our safe method to restart recognition
                        console.log("Restarting recognition after audio response error");
                        safelyStartRecognition("after audio response error");
                      }
                    }, 1000);
                  });
  ...
  const getAssistantResponse = async (text: string, currentConversation: ConversationMessage[]): Promise<void> => {
    try {
      ...
      const generateLocalResponse = (input: string): string => {
        ...
        // Topic explorer and personalized learning
        if (lowerInput.includes('personalized') || lowerInput.includes('personalised') || 
            lowerInput.includes('topic explorer') || lowerInput.includes('discover topics')) {
          speakForBlindUsers("Navigating to personalized learning and topic explorer", true);
        ...
        // Topic recommendation intent
                 ...
                 lowerInput.includes('learning topics')) {
          speakForBlindUsers("Navigating to topic explorer to find personalized recommendations", true);
        ...
        if (lowerInput.includes('hello') || lowerInput.includes('hi jarvis') || lowerInput === 'hi') {
          ...
          // Clear conversation
          setConversation([]);
          localStorage.removeItem('floating_jarvis_conversation');
  ...
  const toggleAssistant = () => {
    const newShowAssistant = !showAssistant;
    setShowAssistant(newShowAssistant);
    
    // When opening or closing the assistant, make sure we don't lose wake word detection
    if (newShowAssistant) {
      // Don't need to stop recognition here - we want it to continue running for wake word detection
      // Just make sure it's active
      if (!isRecognitionActive && recognitionRef.current && recognitionInitialized) {
        setTimeout(() => {
          safelyStartRecognition("assistant open");
        }, 500);
      }
    } else {
      // When closing, ensure recognition is restarted if it's not active
      if (!isRecognitionActive && recognitionRef.current && recognitionInitialized) {
        setTimeout(() => {
          safelyStartRecognition("assistant close");
        }, 500);
      }
    }
  };
